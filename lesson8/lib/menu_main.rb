  ...
  def create_stations
    submenu_block(
      ...
      begin
        ...
        raise unless e.message == 'empty_name'
...
#         
  ...
  def create_wagons
    ...
    submenu_block(
      "Вагоны",
      "\nВведите (номер вагона, тип, кол-во мест(для пасс.вагона=54м) / общий объем(для груз.вагона=68т) (или пункт меню): (11,g,68) (g = #{typeWagon['g'][:ru]}, p = #{typeWagon['p'][:ru]})",
    ) do |result|
      number, type, place  = result.split(",")
      
      if !number || !type || !place
        puts 'Внимание!!! Выполнен некорректный ввод. Повторите, пожалуйста!'
      end       

      begin
        wagon = typeWagon[type][:class_name].new(number, place.to_i)        
      rescue ArgumentError => e
  ...
  def create_trains
    ...
    submenu_block(
      "Поезда",
      "\nВведите (номер, тип) поезда (или пункт меню): (num,g) (g = #{typeTrain['g'][:ru]}, p = #{typeTrain['p'][:ru]})",
    ) do |result|
      ...
      begin
        train = typeTrain[type][:class_name].new(number)
      rescue ArgumentError => e
        case e.message
        when 'incorrect_number'
          puts "Введен некорректный формат (номера) поезда. Повторите ввод!"
        when 'incorrect_type'
          puts "Введен некорректный тип поезда. Повторите ввод!"
        else
          raise
        end
      else
      ...
      case choice
        ...
        when '0'
          ...
          begin
            ...
            case e.message
              when 'incorrect_type'
                puts "Введен некорректный тип вагона (#{typeWagon[wagon.type]})." 
                puts "К поезду (#{train.number}) можно прицепить вагон с типом (#{typeWagon[train.type]}). Повторите, ввод!"
              when 'non_zero_speed'
                puts "Внимание! Невозможно прицепить вагон, поезд (#{train.number}) находится в движении, текущая скорость - #{train.speed} !!!"
              else
                raise
            end 
          else
