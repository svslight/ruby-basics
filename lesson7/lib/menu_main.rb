...
class MenuMain
  ...
  def create_wagons
    ...
    submenu_block(
      "Вагоны",
      "\nВведите (номер вагона, тип) (или пункт меню): (num,g) (g = #{typeWagon['g'][:ru]}, p = #{typeWagon['p'][:ru]})",
    ) do |result|
      number, type = result.split(",") 

      begin
        wagon = typeWagon[type][:class_name].new(number)        
      rescue ArgumentError => e
        raise unless e.message == 'incorrect_type'
        puts "Введен некорректный тип вагона. Повторите ввод!"
      else
        @wagons << wagon
